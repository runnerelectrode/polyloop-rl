...
Status: research done 2026-09-15, build not started. This is the concrete answer to "what do we
...
| Verifier | **Where available** | Repo tests run in the sandbox are the hard reward. Where no tests exist, the same model acts as a judge on (assistant turn, next state) and emits a ternary verdict plus a 1-3 sentence hindsight hint; turns without a next state are dropped (OpenClaw-RL rule). |
...
- **Hint**: same checkpoint as judge, prompt = (assistant turn t, next state t+1 with its role),
  output `\boxed{1|-1}` + `[HINT]...`; 3 votes, keep the longest positive hint, else drop the
  sample. Hint appended to the last user message for the teacher only; the student never sees it.
- **Loss**: per-token advantage `A_t = log pi_T(a_t | s+h) - log pi_old(a_t | s)`, clipped
  ...
  selection where 3 candidate hints exist. KL-to-reference 0; LoRA rank 64.
- **Rollouts**: OPD needs 1 rollout per prompt; GRPO on verifier tasks uses groups of 8.
- **Forgetting**: 30% replay of base-model-distilled prompts (Thinking Machines: IFEval 85 to 45
  without, 83 with); gate on KL-to-base measured on new-task inputs (RL's Razor, R^2 0.71) plus a
  small IFEval / LiveCodeBench slice.

## Evaluation and gate

- **Held-out set**: SWE-rebench monthly split (50-110 fresh tasks, prebuilt images, repo-level
  separation from SWE-smith) plus a fixed 100-task SWE-bench Verified subset for the headline.
  Hold out whole repos, never instances; pool ordered by task hash; canary strings.
- **Repeats**: K = 4 per task per policy. With n = 100 and K = 4 the 95% CI half-width is about
  7.6 pp; +3 pp needs 700-1100 tasks (Miller's paired formula). So a single nightly gate cannot
  certify +3 pp. The gate therefore uses: paired mean delta with tie band, regression count cap,
...
2601.19897; Thinking Machines on-policy distillation post; Whitney OPSD case studies; Miller /
